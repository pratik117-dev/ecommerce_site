...
def add_cart(request, product_id):
    ...
    try:
        cart = Cart.objects.get(cart_id=_cart_id(request))  # Get the cart using cart_id from session
    ...
    if is_cart_item_exist:
        cart_item = CartItems.objects.filter(product=product, quantity= 1, cart=cart)
